...
int m = 3;
int n = 4;
double minLimitRandom = -10;
double maxLimitRandom = 10;
Random random = new Random();
double array = matrix;
double [,] matrix(int m, int n, double minLimitRandom, double maxLimitRandom)
{
    ...
    for (int i = 0; i <m; i++)
    {
        for (int j = 0; j < n; j++)
        {
            matrix[i, j] = new Random().Next(-10, 10)+random.NextDouble();
